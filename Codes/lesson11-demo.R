...
freq[tail(ord)] #Most frequent terms .. tail fetch last 6 terms & head fetch first 6 terms
...
corpus.tranf <- tm_map(corpus.tranf, removeWords, c("since", "let", "yes", "every", "yeah")) 
# Perform Stemmings
...
train.doc <- dtm[c(1,2,5,6),] # documents(1,2,5,6) Dataset for which classification is already known
